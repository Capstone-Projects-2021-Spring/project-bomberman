...
public class Bomber extends Player {
    ...
    private void Generate_movement() {
        ...
        if (this.resting && !this.bomb_planted) {
            // Find a place to move -- Loop thorough every breakable wall's y position and
            // find the closest one, ie current character position - walls y position

            for (int i = 0; i <= GameObjectCollection.tileObjects.size() - 1; i++) {

                if (GameObjectCollection.tileObjects.get(i).isBreakable()
                        && this.position.y - GameObjectCollection.tileObjects.get(i).position.y > 0.0) {

                    // System.out.println("Walls x position " +
                    // GameObjectCollection.tileObjects.get(i).position.x);
            ...
            if (Walls.size() > 0) {
                ...
                // System.out.println("Closest breakable walls y position = " + (-Walls.get(0) +
                // this.position.y));
    ...
    // This function is meant to be called by a for loop - the iterator is the
    // distance from the start to location
    ...
    /**
     * 1. Find max X and Y 2. Find all Tile Objects between 1 and MaxX-1 && 1 and
     * MaxY-1 (X between 1 and 13 and Y between 1 and 11) 3. Create an number list
     * to represent the different tiles (0 = empty space, 1 = Soft Walls, 2 = Hard
     * Walls, 3 = Ourselves) 4. Find all the empty space
     */

    public void print_tiles() {
        for (int i = 0; i <= GameObjectCollection.tileObjects.size() - 1; i++) {
            System.out.println("Object : " + (GameObjectCollection.tileObjects.get(i)) + "        X position: "
                    + GameObjectCollection.tileObjects.get(i).position.x / 32.0 + "       Y position: "
                    + GameObjectCollection.tileObjects.get(i).position.y / 32.0);
    ...
    public ArrayList<TileObject> drawMapTileObj(float xMax, float yMax) {
        ...
        for (int i = 1; i <= GameObjectCollection.tileObjects.size() - 1; i++) {
            if (GameObjectCollection.tileObjects.get(i).position.y / 32 >= 1
                    && GameObjectCollection.tileObjects.get(i).position.y / 32 <= yMax - 1
                    && GameObjectCollection.tileObjects.get(i).position.x / 32 >= 1
                    && GameObjectCollection.tileObjects.get(i).position.x / 32 <= xMax - 1) {
        ...
        /*
         * for(int j = 0; j <= tileObj.size() - 1; j++){ System.out.println("Object : "
         * + (tileObj.get(j))+ "        X position: "+ tileObj.get(j).position.x/32.0 +
         * "       Y position: " +tileObj.get(j).position.y/32.0 );
    ...
    /*
     * 1. Split the array into rows 2. Every MaxY -1 will be a new row
     */

    /*
     * Step1 ) Find our player position in regards to the map array We have our
     * player x and y /32
     */
    ...
    private void whereToGO() {
        ...
        // System.out.println("Closest Box: " +
        ...
        if (this.resting && tileObjInt.contains(1)) {
            if (closest[1] % 2 != 0) { // Turning on the horozontal direction
                ...
                if (closest[1] == (int) yPlayerSpawn) { // this means they are on the same row (X direction)
                    ...
                    if (xDiff > 0) { // right
                        // xDiff -=1 ; //causes the bot to move to the right side of the box and not the
                        // left side

                        for (int i = 0; i <= xDiff * tile / moveSpeed + 5; i++) { // +3 was nessicary to cause a
                                                                                  // collision.
                            if ((int) (this.position.x / 32) > ((-closest[0] + (int) xPlayerSpawn)) + 1
                                    && this.resting) {
                                this.toggleRightPressed();
                        ...
                        for (int i = 0; i <= xDiff * tile / moveSpeed + 3; i++) { // +3 was nessicary to cause a
                                                                                  // collision.
                ...
                else if (closest[1] != yPlayerSpawn) { // if target box and bomber are not on the same row we need to
                                                       // work in the y direction
                    int yDiff = closest[1] - (int) yPlayerSpawn; // if diff is positive we go down, if diff is negative
                                                                 // we go up
                    ...
                    if (yDiff > 0) { // DOWN
                        ...
                        for (int i = 0; i <= yDiff * tile / moveSpeed + 5; i++) { // if this is not 5 it will cause a
                                                                                  // collision on the corner the bomb
                                                                                  // will be planted a tile early
                            if ((int) (this.position.y / 32) > ((-closest[1] - 1 + (int) yPlayerSpawn))
                                    && this.resting) {
                                this.toggleUpPressed();
                                this.move();
                                this.resting = false;
    ...
    /**
     * Bombs act as walls if the bomber is not already within the a certain distance
     * as the bomb. This is also the big and ugly kicking logic. Touching this code
     * is very dangerous and can introduce bugs to the kicking logic including
     * stopping the bomb from moving. (ie. if the bomber is not standing on the
     * bomb)
     * 
