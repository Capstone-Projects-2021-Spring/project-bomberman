...
public class GamePanel extends JPanel implements Runnable {
    ...
    GamePanel(String filename, PrintWriter out, BufferedReader in, int player, String[] maps, JTextArea messageArea) {// online
                                                                                                                      // multiplayer
    ...
    /**
     * Loads the map file into buffered reader or load default map when no file is
     * given. The file should be a file with strings separated by commas ",".
     * Preferred .csv file.
     * 
...
//                        break;
                ...
                if (mapLayout.get(y).get(x).equals("S")) { // Soft wall; breakable
                    ...
                    if (!mapLayout.get(y).get(x).contains("-")) {
                        if (Integer.parseInt(mapLayout.get(y).get(x)) == player + 1) {
                            BufferedImage[][] sprMapP1 = ResourceCollection.SpriteMaps.PLAYER_1.getSprites();
                            Bomber player1 = new Bomber(new Point2D.Float(x * 32, y * 32 - 16), sprMapP1, GameType, out,
                                    player + 1);
                            PlayerController playerController1 = new PlayerController(player1, this.controls1);
                            this.addKeyListener(playerController1);
                            if (player < 4) {
                            ...
                            BufferedImage[][] sprMapP2 = ResourceCollection.SpriteMaps.PLAYER_2.getSprites();
                            Bomber player2 = new Bomber(new Point2D.Float(x * 32, y * 32 - 16), sprMapP2, GameType, out,
                                    Integer.parseInt(mapLayout.get(y).get(x)));
                            PlayerController playerController2 = new PlayerController(player2, this.controls2);
                            this.addKeyListener(playerController2);
    ...
    /**
     * The game loop. The loop repeatedly calls update and repaints the panel. Also
     * reports the frames drawn per second and updates called per second (ticks).
     */
    @Override
    public void run() {
     ...
     * Checks collisions between every two game objects. Deletes game objects that
     * are marked for deletion. Checks if a player is a winner and updates score,
     * then reset the map.
     */
    private void update() {
            ...
            for (int objIndex = 0; objIndex < GameObjectCollection.gameObjects.get(list).size();) {
                ...
                if (obj.isDestroyed()) {
                    ...
                    for (int list2 = 0; list2 < GameObjectCollection.gameObjects.size(); list2++) {
                        for (int objIndex2 = 0; objIndex2 < GameObjectCollection.gameObjects.get(list2)
                                .size(); objIndex2++) {
        ...
        // Check for the last bomber to survive longer than the others and increase
        // score
        // Score is added immediately so there is no harm of dying when you are the last
        // one
        // Reset map when there are 1 or less bombers left
        if (!this.gameHUD.matchSet) {
            this.gameHUD.updateScore();
        } else {
            // Checking size of array list because when a bomber dies, they do not
            // immediately get deleted
            // This makes it so that the next round doesn't start until the winner is the
            // only bomber object on the map
    ...
    private void updateSingle() {
        ...
        if (!this.gameHUDSingle.matchSet) { // check if the game has not already been done, if not check for score kills
            ...
            // Checking size of array list because when a enemy dies, they do not
            // immediately get deleted
            ...
            } else if (GameObjectCollection.bomberObjects.isEmpty()) { // this should be reset the map back to stage 1
...
class GameController implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_F1) {
            ...
            Object[][] controls = { { "Up", "Up", "W", "T", "I" }, { "Down", "Down", "S", "G", "K" },
                    { "Left", "Left", "A", "F", "J" }, { "Right", "Right", "D", "H", "L" },
                    { "Bomb", "/", "E", "Y", "O" }, { "", "", "", "", "" }, { "Help", "F1", "", "", "" },
                    { "Reset", "F5", "", "", "" }, { "Exit", "ESC", "", "", "" } };
